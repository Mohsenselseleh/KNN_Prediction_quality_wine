...
install.packages("class",   repos = c("http://rstudio.org/_packages",   "http://cran.rstudio.com"))
install.packages("gmodels",   repos = c("http://rstudio.org/_packages",   "http://cran.rstudio.com"))
library(corrplot)
library(class)
library(gmodels)
```
...
summary(wine_n)
```
8. Divide the data to training and testing groups  
```{r}
set.seed(1)
index <- sample(1:nrow(wine_n), 0.65 *nrow(wine_n))
wine_train <- wine_n[index,]
...
summary(wine_test_labels)
```
9. Use the KNN algorithm to predict quality of wine using its attributes 
